...
const refreshAccessTokens = async (refresh_time) => {
  ...
  console.log(
    `[${dateString}] refreshing onedrive access tokens, refreshing every ${
    ...
    await fetch(`${process.env.BASE_REQUEST_URL}/api/onedrive/getusers`)
      ...
      .then(async (data) => {
        for (let i = 0; i < data.length; i++) {
          try {
            logger.trace(
              `{refreshAccessTokens} Refreshing access token for user ${data[i].uid}`
            )
            await fetch(
              `${process.env.BASE_REQUEST_URL}/api/onedrive/refresh/${data[i].uid}`,
              { method: "PUT" }
            )
            ...
            logger.trace(
              `{refreshAccessTokens} Access token failed for user ${data[i].uid}`
            )
          }
        }
      })
...
const executeOneDriveFlows = async (refresh_time) => {
  ...
  console.log(
    `[${dateString}] running onedrive flows, refreshing every ${
      refresh_time / 1000 / 60
    } minutes`
  )
  ...
  for (let i = 0; i < pastData.length; i++) {
    ...
    await fetch(
      `${process.env.BASE_REQUEST_URL}/api/userbyid/${pastData[i].userid}`
    )
      .then((response) => response.json())
      .then(async (data) => {
        const userData = data
        ...
        await fetch(`${process.env.BASE_REQUEST_URL}/api/onedrive/children`, {
          ...
          .then(async (data) => {
            ...
            if (difference.length !== 0) {
              ...
              await fetch(
                `${process.env.BASE_REQUEST_URL}/api/notion/${userData.id}`
              )
                .then((response) => response.json())
                .then(async (data) => {
                  // (4)
                  for (let j = 0; j < difference.length; j++) {
                    const notionPageInfo =
                      await notionImports.createPageInDatabase(
                    ...
                    await fetch(
                      `${process.env.BASE_REQUEST_URL}/api/todoist/${userData.id}`
                    )
                      .then((response) => response.json())
                      .then((data) => {
                        // (5)
                        Todoist.createTask(
                          ...
                          data.sections.find(
                            (section) =>
                              section.name ===
                              difference[j].parentReference.name
                          ).id,
                          [
                            data.labels.find((label) => label.name === "review")
                              .id,
                          ],
                          "today"
                        )
              ...
              //(6)
              await fetch(
                `${process.env.BASE_REQUEST_URL}/api/flows/pastfiles/${pastData[i].id}`,
                {
                  method: "PUT",
                  headers: {
                    "Content-Type": "application/json",
