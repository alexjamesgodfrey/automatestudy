...
const getAccessToken = async (code, uid) => {
  ...
  try {
    await fetch("https://login.microsoftonline.com/common/oauth2/v2.0/token", {
      ...
      .then(async (data) => {
        logger.trace(
          "OneDrive access token received from Microsoft for user " + uid
        )
        const accessToken = data.access_token
        //we do not want to add a null access token
        if (data.access_token) {
          await fetch(
            `${process.env.BASE_REQUEST_URL}/api/onedrive/store/${uid}`,
            {
              method: "PUT",
              headers: {
    ...
    logger.error(
      "OneDrive access token could not be fetched and added to database for user " +
        uid +
        "\nError message: " +
        err.message
    )
...
const getAccessTokenFromRefresh = async (refresh, uid) => {
  ...
  try {
    await fetch("https://login.microsoftonline.com/common/oauth2/v2.0/token", {
      ...
      .then(async (data) => {
        logger.trace(
          "[Refresh] OneDrive access token received from Microsoft for user " +
            uid
        )
        //we do not want to add a null access token
        if (data.access_token) {
          await fetch(
            `${process.env.BASE_REQUEST_URL}/api/onedrive/updatetoken/${uid}`,
            {
              method: "PUT",
              headers: {
                "Content-Type": "application/json",
...
module.exports = function (app) {
  ...
  app.get("/api/onedrive/getusers", async (req, res) => {
    try {
      const oneDriveUsers = await pool.query(
        "SELECT * FROM users WHERE cloud='OneDrive'"
      )
  ...
  app.put("/api/onedrive/initialize/:code/:uid", async (req, res) => {
    try {
      ...
      logger.trace(
        "[Intiialization] OneDrive access token fetch requested for user " + uid
      )
      ...
      logger.error(
        "[Intiialization] OneDrive access token fetch failed for user " +
          uid +
          "\n error message: " +
          err.message
      )
  ...
  app.put("/api/onedrive/refresh/:uid", async (req, res) => {
    try {
      const { uid } = req.params
      logger.trace(
        "[Refresh] OneDrive access token fetch requested for user " + uid
      )
      ...
      const { uid } = req.params
      logger.error(
        "[Refresh] OneDrive access token fetch failed for user " +
          uid +
          "\n error message: " +
          err.message
      )
  ...
  app.put("/api/onedrive/store/:uid", async (req, res) => {
    try {
      ...
      const changeOnedrive = await pool.query(
        "UPDATE users set cloud='OneDrive', cloudaccess=$1, cloudrefresh=$2 WHERE uid = $3",
        [access_token, refresh_token, uid]
      )
      logger.trace(
        "[Intiialization] OneDrive access token successfully added to database for user " +
          uid
      )
    } catch (err) {
      const { uid } = req.params
      logger.error(
        "[Intiialization] OneDrive access token could not be added to database for user " +
          uid +
          "\n error message: " +
          err.message
      )
  ...
  app.put("/api/onedrive/updatetoken/:uid", async (req, res) => {
    try {
      ...
      await pool.query("UPDATE users set cloudaccess=$1 WHERE uid = $2", [
        access_token,
        uid,
      ])
      logger.trace(
        "[Refresh] OneDrive access token successfully added to database for user " +
          uid
      )
    } catch (err) {
      const { uid } = req.params
      logger.error(
        "[Refresh] OneDrive access token could not be added to database for user " +
          uid +
          "\n error message: " +
          err.message
      )
  ...
  app.put("/api/onedrive/children", async (req, res) => {
    try {
      ...
      if (childid) {
        await fetch(
          `${process.env.BASE_MICROSOFT_URL}/drives/${driveid}/items/${childid}/children`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${access_token}`,
      ...
      await fetch(
        `${process.env.BASE_MICROSOFT_URL}/drives/${driveid}/root/children`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${access_token}`,
